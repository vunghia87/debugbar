    ...
    <div class="card-header d-flex align-items-center justify-content-between p-0">
        <ul class="nav nav-pills">
            ...
            <?php if (!empty($data['exceptions']['count'])): ?>
                <li class="nav-item"><a href="#exception" class="nav-link">Exception
                        (<?= $data['exceptions']['count'] ?>)</a>
                </li>
            <?php endif; ?>
            <?php if (!empty($data['pdo']['statements'])): ?>
                <li class="nav-item"><a href="#database" class="nav-link">Queries
                        (<?= count($data['pdo']['statements']) ?>)</a>
                </li>
            <?php endif; ?>
            <?php if (!empty($data['messages']['count'])): ?>
                <li class="nav-item"><a href="#message" class="nav-link">Message (<?= $data['messages']['count'] ?>)</a>
                </li>
            <?php endif; ?>
            <?php if (!empty($data['memcache']['count'])): ?>
                <li class="nav-item"><a href="#memcache" class="nav-link">Memcache (<?= $data['memcache']['count'] ?>)</a>
                </li>
            <?php endif; ?>
            <?php if (!empty($data['command']['count'])): ?>
                <li class="nav-item"><a href="#command" class="nav-link">Commmand (<?= $data['command']['count'] ?>)</a>
                </li>
            <?php endif; ?>
        </ul>
    </div>
